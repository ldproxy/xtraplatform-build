...
public class Docs {
  ...
  DocRef findTypeRef(String qualifiedName) {
    return layers.stream()
        .flatMap(layer -> getTypes(layer, typeDocs -> Objects.equals(typeDocs.qualifiedName, qualifiedName)))
        .findFirst()
        .orElseThrow(() -> new IllegalArgumentException(
            "Referenced type could not be resolved: " + qualifiedName));
  }
  ...
  Stream<DocRef> getTypes(LayerDocs layer, Predicate<TypeDocs> predicate) {
    ...
    return layer.modules.values()
        .stream()
        .flatMap(module -> module.getTypes(predicate)
  ...
  List<DocFile> getDocFiles() {
    return streamTypes()
        ...
        .map(typeRef -> DocFile.from(typeRef, typeRef.getType()
            .getAnnotation(DocFile.ANNOTATION)
            .get()
        ))
        .collect(Collectors.toList());
  }
  ...
  List<DocFilesTemplate> getDocFilesTemplates() {
    return streamTypes()
        ...
        .map(typeRef -> DocFilesTemplate.from(typeRef, typeRef.getType()
