...
class DocFilesTemplate {
  ...
  private static final Type LIST_I18N = new TypeToken<List<DocI18n>>() {}.getType();
  private static final Type LIST_TABLE = new TypeToken<List<DocTable>>() {}.getType();
  private static final Type LIST_VAR = new TypeToken<List<DocVar>>() {}.getType();
  ...
  private DocFilesTemplate(
      DocRef docRef,
      String path,
      Optional<String> stripPrefix,
      Optional<String> stripSuffix,
      CaseFormat caseFormat,
      List<DocI18n> template,
  ...
  static DocFilesTemplate from(DocRef docRef, AnnotationDocs docFilesTemplateDef) {
    ...
    String path =
        docFilesTemplateDef
            ...
            .orElse(
                type.qualifiedName
                    .substring(0, type.qualifiedName.lastIndexOf('.'))
                    .replaceAll("\\.", "/"));
    ...
    List<DocTable> tables =
        docFilesTemplateDef
            .getAttributeAsJson("tables")
            .map(t -> (List<DocTable>) GSON.fromJson(t, LIST_TABLE))
            .orElse(List.of());
    List<DocVar> vars =
        docFilesTemplateDef
            .getAttributeAsJson("vars")
            .map(t -> (List<DocVar>) GSON.fromJson(t, LIST_VAR))
            .orElse(List.of());

    return new DocFilesTemplate(
        docRef, path, stripPrefix, stripSuffix, caseFormat, template, tables, vars);
  }
  ...
  String getName(DocRef docRef) {
    String name =
        docRef
            .getDocTag("title")
            .findFirst()
            .map(title -> title.toLowerCase().replaceAll(" ", "_"))
            .orElse(docRef.getType().getName());
